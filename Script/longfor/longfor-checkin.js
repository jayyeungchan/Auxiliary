...
// 常量定义
const $ = new Env('龙湖签到');
const tokenKey = 'longhu_token';
const signUrl = 'https://api.longhu.net/user/sign';
    ...
    try {
        ...
        if (response.code === 0) {
            ...
            $.msg(
                $.name, 
                '签到成功 ✅', 
                `连续签到: ${data.days}天\n获得积分: +${data.points}\n当前积分: ${data.totalPoints}`
            );
        } else {
            $.msg($.name, '签到失败 ❌', response.message || '未知错误');
        }
    } catch (e) {
        $.log(`签到请求失败: ${e}`);
